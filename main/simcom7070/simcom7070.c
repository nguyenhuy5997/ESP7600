...
bool mqttConnect(client clientMqtt)
{
	...
	sprintf(buff_send, "AT+SMCONF=\"USERNAME\",\"%s\"", clientMqtt.clName);
	while(retry--)
	{
		_sendAT(buff_send);
		res = ___readSerial(1000, "OK");
	...
	sprintf(buff_send, "AT+SMCONF=\"PASSWORD\",\"\%s""", clientMqtt.clPass);
	while(retry--)
	{
		_sendAT(buff_send);
		res = ___readSerial(1000, "OK");
	...
	sprintf(buff_send, "AT+SMCONF=\"CLIENTID\",\"%s\"", clientMqtt.clID);
	while(retry--)
	{
		_sendAT(buff_send);
		res = ___readSerial(1000, "OK");
	...
	sprintf(buff_send, "AT+SMCONF=\"KEEPTIME\",%d", clientMqtt.kta);
	while(retry--)
	{
		_sendAT(buff_send);
		res = ___readSerial(1000, "OK");
	...
	sprintf(buff_send, "AT+SMCONF=\"CLEANSS\",%d", clientMqtt.cleaness);
	while(retry--)
	{
		_sendAT(buff_send);
		res = ___readSerial(1000, "OK");
...
bool readGPS(gps *gps)
{
	...
	char *gps_buff = "";
	_sendAT("AT+CGNSINF");
	res = ___readSerial(5000, "+CGNSINF:");
	if(res == AT_OK)
	{
